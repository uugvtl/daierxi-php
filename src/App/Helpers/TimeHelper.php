...
class TimeHelper extends BaseHelper
{
     ...
     * @return int                  一般是一周秒数
     */
    public function getOffsetDeliveryTime()
    {
        return 7*24*60*60;
    }
     ...
     *
     */
    public function getShowDateTimeFormat($timestamp,$format='Y-m-d H:i:s')
    {
	 ...
	 */
	public function getDate($d=false,$gmt=false)
	{
		if($d===false)
			$d=time();
		if($gmt)
	 ...
	 * @return boolean true if valid date, semantic check only.
	 */
	public function isValidTime($h,$m,$s,$hs24=true)
	{
	 ...
	 */
	public function formatDate($fmt,$d=false,$is_gmt=false)
	{
		if ($d === false)
	 ...
	 */
	public function getTimestamp($hr,$min,$sec,$mon=false,$day=false,$year=false,$is_gmt=false)
	{
		if ($mon === false)
